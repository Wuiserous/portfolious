    ...
    return (
        <div className="w-[50%] h-[100%] flex flex-col justify-center items-center border-l-[1px] border-r-[1px] border-gray-700">
            <div className={`flex sticky top-0 w-full h-fit justify-between items-center border-l-[1px] border-r-[1px] border-gray-700 bg-transparent ${styles.navBar}`}>
                <button 
                    className={`flex-1 hover:bg-orange-500 transition-all duration-400 ease-in-out text-center py-2 ${activeTab === 'All' ? 'border-b-2 border-orange-500' : 'border-b-2 border-transparent'}`} 
                    onClick={() => handleTabClick('All')}
                >
                    All
                </button>
                <button 
                    className={`flex-1 hover:bg-orange-500 transition-all duration-200 ease-in-out text-center py-2 ${activeTab === 'Following' ? 'border-b-2 border-orange-500' : 'border-b-2 border-transparent'}`} 
                    onClick={() => handleTabClick('Following')}
                >
                    Following
                </button>
                <button 
                    className={`flex-1 hover:bg-orange-500 transition-all duration-200 ease-in-out text-center py-2 ${activeTab === 'Jobs' ? 'border-b-2 border-orange-500' : 'border-b-2 border-transparent'}`} 
                    onClick={() => handleTabClick('Jobs')}
                >
                    Jobs
                </button>
                <button 
                    className={`flex-1 hover:bg-orange-500 transition-all duration-200 ease-in-out text-center py-2 ${activeTab === 'Projects' ? 'border-b-2 border-orange-500' : 'border-b-2 border-transparent'}`} 
