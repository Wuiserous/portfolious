...
export default function Middle({ posts }: { posts: Post[] }) {
    return (
        <div className="w-[50%] h-[100%] flex flex-col justify-center items-center border-r-[1px] border-gray-700">
            <div className="flex sticky top-0 gap-2 w-full h-fit justify-between items-center border-b-[1px] border-gray-700 bg-transparent" className={styles.burgerIcon}>
                for you
            </div>
            <div className="w-[100%] h-[100vh] overflow-y-scroll flex flex-col gap-2 p-2 bg-transparent no-scrollbar scroll-smooth">
           {posts.map((post) => (
